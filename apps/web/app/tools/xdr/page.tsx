...
import { AlertCircle, CheckCircle, Copy, Trash2, Code, ArrowRightLeft, ShieldAlert, Terminal } from "lucide-react";
...
// Issue #1107: ready-to-run Stellar CLI snippets
import { XDR_CLI_TARGETS, buildXdrCliCommand, type XdrCliTarget } from "@/lib/xdr-cli-command";
...
export default function XdrToolsPage() {
  ...
  // Issue #1107: CLI snippet target for the "Copy as CLI Flag" action
  const [cliTarget, setCliTarget] = useState<XdrCliTarget>("xdr-decode");
  ...
  // Issue #1107: the CLI snippet for the current input and target pair
  const cliCommand = buildXdrCliCommand(cliTarget, decodeInput, { type: typeHint });
          ...
          <div className="space-y-4">
            <Card className="flex h-full flex-col">
              ...
              <CardContent className="flex-1 space-y-4">
                ...
                {/* Issue #1107: turn the pasted XDR into a runnable Stellar CLI command */}
                <div className="space-y-2 rounded-md border border-dashed p-3">
                  <div className="flex flex-wrap items-end gap-2">
                    <div className="min-w-[190px] flex-1 space-y-1.5">
                      <Label htmlFor="cli-target">Copy as CLI flag</Label>
                      <Select value={cliTarget} onValueChange={(value) => setCliTarget(value as XdrCliTarget)}>
                        <SelectTrigger id="cli-target">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          {XDR_CLI_TARGETS.map((target) => (
                  ...
                  <code
                    data-testid="cli-command-preview"
                    className="block break-all rounded-md bg-muted px-2 py-1.5 font-mono text-[11px] text-muted-foreground"
                  >
                    {cliCommand || "Paste an XDR string to build a command"}
                  </code>
                </div>
                {decodeError && (
